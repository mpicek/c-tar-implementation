...
void get_flag_of_file(FILE *f, char typeflag[], long long file_length) {
  read_string(f, TYPEFLAG_LOCATION, typeflag, TYPEFLAG_LENGTH,
              file_length); // read TYPEFLAG
}
...
void print_file(char name[], struct files_to_process *ftprocess,
                bool delete_from_files_to_process) {
  if (ftprocess->defined) {
    for (int i = 0; i < ftprocess->number; ++i) {
...
bool process_one_file_and_jump(FILE *f, struct action_info action,
                               struct files_to_process ftprocess,
                               long long file_length) {
  ...
  // read_string(f, SIZE_LOCATION, sizeStr, SIZE_LENGTH, file_length); // read
  // SIZE
  ...
  if (action.list) {
    ...
    if ((typeflag[0] - '0' != REGULAR_FILE_FLAG &&
         typeflag[0] - '0' != DIRECTORY_FILE_FLAG) &&
        (name[0] != 0)) {
      errx(2, "Unsupported header type: %d", typeflag[0]);
    }
    if (action.verbose)
...
void process_action(struct action_info action,
                    struct files_to_process ftprocess) {
...
void process_tar_archive(struct action_info action,
                         struct files_to_process ftprocess) {
  if (action.list && action.extract) {
    ...
    if (action.action_defined) {
      if (action.file) {
        ...
        errx(2,
             "tar: Refusing to read archive contents from terminal (missing -f "
             "option?)\ntar: Error is not recoverable: exiting now");
      }
    } else {
